...
window.onload = function() {
    let selectLocationEl = document.getElementById("locationSelect");
    let selectTypeEl = document.getElementById("parkSelect");
    let descriptionEl = document.getElementById("description");

    // Populate dropdown for location // 
    let populateLocationDropdown = (selectEl, array) => {
        for (let i=0; i< array.length; i++) {
            let optionData = array[i];
            let optionEl = new Option(optionData, optionData);
            selectEl.appendChild(optionEl);
        }
    };

    // Populate dropdown for park type //
    let populateTypeDropdown = (selectEl, array) => {
        for (let i=0; i<array.length; i++) {
            let optionData = array[i];
            let optionEl = new Option(optionData, optionData);
            selectEl.appendChild(optionEl);
    ...
    // Displays park info based on selected location // 
    let displayParkInfoByLocation = () => {
        let selectLocation = selectLocationEl.value;
        descriptionEl.innerHTML = "";

        if(selectLocation === "Select by location: ") {
            ...
            let parksInLocation = nationalParksArray.filter((park) => park.state === selectLocation);
    ...
    // Display park info based on selected park //
    let displayParkInfoByType = () => {
        let selectedType = selectTypeEl.value;

        let parksOfType = nationalParksArray.filter((park) => {
            return park.LocationName.toLowerCase().includes(selectedType.toLowerCase());
        });

        if (parksOfType.length > 0) {
            parksOfType.forEach((park) => {
                const parkName = park.LocationName;
                let listEl = document.createElement("li");
                listEl.innerHTML = parkName;
                descriptionEl.appendChild(listEl);
    ...
    selectLocationEl.onchange = displayParkInfoByLocation;
    selectTypeEl.onchange = displayParkInfoByType;

populateLocationDropdown(selectLocationEl, locationsArray);
populateTypeDropdown(selectTypeEl, parkTypesArray);
};
